...
class MascotAssetContractTest {
    ...
    @Test
    fun animatedPipAtlasIsVerifiedTransparentBoundedAndPrepared() {
        val context = ApplicationProvider.getApplicationContext<Context>()
        val bytes = context.assets.open("mascots/pip_animated.webp").use { it.readBytes() }
        val manifest = context.assets.open("mascots/pip_animated.json")
        ...
        assertEquals(1_310_720, bitmap.width * bitmap.height * 4)
        assertTrue("Compressed Pip atlas unexpectedly large", bytes.size < 180_000)

        assertTrue("Integrated atlas must be explicitly approved", manifest.getBoolean("approved_for_runtime"))
        val clips = manifest.getJSONObject("clips")
        for (name in listOf("idle", "jump", "fall", "happy")) {
            val frames = clips.getJSONArray(name)
            assertEquals("$name must keep the reviewed five-frame cadence", 5, frames.length())
        ...
        // Exercises both the legacy fallback atlas and the new animated Pip integrity checks.
